...
export class SignLanguageComponent implements AfterViewInit, OnInit {
  ...
  constructor(
    private languagesService: SignLanguagesService,
  ...
  getLanguages() {
    this.languagesService.getSignLanguages().subscribe(
      data => {
        this.languages = data;
  ...
  placeIcon() {
    this.languagesService.getSignLanguages().subscribe(data => {
  ...
  setIcon() {
    ...
    for (let i = 0; i < this.filteredData.length; i++) {
      if (this.filteredData[i].coordinates !== undefined && this.filteredData[i].coordinates != null) {
        ...
        const coordstring = this.filteredData[i].coordinates.split(',');
  ...
  onLanguage() {
    this.languagesService.getLanguageDetail(this.markerSelectedContent.code).subscribe((data: any) => {
